...
function getAudio($: CheerioAPI, message: MessageSelection, options: StaticProxyOptions): string {
  ...
  const docAudio = docWrap.find('audio, [data-audio], source')
  const docLink = docWrap.find('a[href]').first()
  const docSrc = docAudio.attr('src')
    || docAudio.attr('data-src')
    || docAudio.attr('data-audio')
    || docLink.attr('href')
  ...
  if (voiceSrc) {
    ...
    const audioSrc = audioEl.attr('src')
      || audioEl.find('audio, source').attr('src')
      || audioEl.find('a[href]').first().attr('href')
      || message.find('.tgme_widget_message_document_wrap a[href]').first().attr('href')
    if (!audioSrc)
      return ''
